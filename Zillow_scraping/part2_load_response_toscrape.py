...
class Zillowscraper():
    ...
    def parse(self, html):
        ...
        for card in cards:
            try:
                ba = card.find('ul', {'class': 'list-card-details'}).findAll('li')[1].text
            except:
                ba = 'NA'

            try:
                sqft = card.find('ul', {'class': 'list-card-details'}).findAll('li')[2].text
            except:
                sqft = 'NA'
            ...
            self.results.append({
                ...
                'beds': card.find('ul', {'class': 'list-card-details'}).findAll('li')[0].text,
                'bathrooms': ba,
                'size': sqft,
                'images': img
                                })
    ...
    def run(self):
        for page in range(1, 5):
            params = {
                'searchQueryState': """{"pagination":{"currentPage":%s,"usersSearchTerm":"New York, NY","mapBounds":{"west":-74.59891294883812,"east":-73.35700005116189,"south":40.454016029555525,"north":40.956841215490435},]"regionSelection":[{"regionId":6181,"regionType":6}],"filterState":{},"isListVisible":true,"isMapVisible":false""" % page}
        # part1
